param([Parameter()][string]$Environment)
...
if ($Environment -eq "HCG")
{
    $query_serverlist = "Select [name]=[server] from dbo.DBA_HCG_Server_List WHERE [BLOCKINGCHECK]=1"
}
else
{
    $query_serverlist = "Select [name]=[server] from dbo.DBA_Server_List WHERE [BLOCKINGCHECK]=1"
}
